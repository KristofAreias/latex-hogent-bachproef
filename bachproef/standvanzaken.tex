...
\section{Productherkenning als detectie- en identificatieprobleem}
...
\item verschillen tussen varianten van hetzelfde product zijn vaak klein (zoals zero-varianten).
\end{itemize}

\textcite{FRANCO2017163} tonen aan dat een belangrijke aanname uit klassieke objectdetectie vaak niet klopt in de retailcontext. Meestal gaat men ervan uit dat trainingsdata goed overeenkomt met de situatie in de praktijk. In supermarkten is dat echter vaak niet het geval. Trainingsbeelden worden genomen in gecontroleerde omstandigheden, terwijl beelden in winkels veel variatie bevatten. Daardoor ontstaat er een verschil tussen training en gebruik in de praktijk.

Productherkenning moet niet alleen goed omgaan met overlappende producten en verschillen in grootte, maar ook met producten die sterk op elkaar lijken.

\section{Datasetkarakteristieken en representativiteit}
...
De kwaliteit en representativiteit van datasets is dus geen detail, maar heeft een grote invloed op hoe goed modellen presteren en is hierdoor een enorm belangrijk onderdeel in de proef.

\section{Klassieke computer vision benaderingen}
...
\item Bag-of-Visual-Words representaties; voor een visuele representatie zoals een verzameling van de belangrijkste visuele kenmerken.
\end{itemize}
...
\section{CNN-gebaseerde objectdetectie en embedding-methoden}
...
\section{Transformer-gebaseerde detectiearchitecturen}

Recente ontwikkelingen in objectdetectie maken steeds vaker gebruik van transformer-modellen als alternatief voor de klassieke methodes.

Een bekend voorbeeld is DETR (Detection Transformer). Dit model bekijkt objectdetectie op een nieuwe manier: in plaats van veel overlappende voorspellingen te maken en die achteraf op te schonen, probeert het meteen een vaste set objecten in een afbeelding te voorspellen \autocite{carion2020}. Daarbij leert het model direct welke voorspelling bij welk echt object hoort.

Een verbeterde versie hiervan is Deformable DETR. Dit model werkt efficiënter doordat het zich beter kan focussen op relevante delen van een afbeelding \autocite{zhu2021}. Hierdoor leert het sneller en presteert het beter, vooral bij kleine objecten die moeilijk te detecteren zijn. Dit is bijzonder relevant voor supermarktbeelden met hoge objectdensiteit.

Transformer-gebaseerde modellen bieden theoretische voordelen bij complexere beelden, maar vereisen ook grotere trainingsdatasets.

\section{Fine-grained classificatie op SKU-niveau}
...
\section{Multimodale benaderingen met beeld en tekst}
...
\section{Generaliseerbaarheid en domeinadaptatie}
...
\section{Evaluatiekaders voor retaildetectie}
...
\section{Geïdentificeerde tekortkomingen in de literatuur}
\label{sec:tekortkomingen_lit}

Uit de literatuur blijkt dat er verschillende belangrijke gaten bestaan in onderzoek naar automatische productherkenning in supermarkten.

Veel studies werken met gecontroleerde datasets die niet lijken op echte winkelomstandigheden, waar producten dicht op elkaar staan, belichting varieert en verpakkingen overlappen \autocite{MELEK2024108452}.

Er worden ook zelden verschillende modellen onder identieke omstandigheden vergeleken. De meeste studies testen één model of pipeline \autocite{TONIONI201810733}, waardoor we niet goed kunnen zien welk type model echt beter presteert.

Onderzoek naar de invloed van beeldfactoren zoals camerahoek, resolutie, afstand tot het rek of productdichtheid komt niet veel voor. Domeinverschuiving wordt wel genoemd \autocite{FRANCO2017163}, maar vaak ontbreekt kwantitatief bewijs over welke factoren detectieprestaties het meest beïnvloeden.

De evaluatie van nieuwe of onbekende producten blijft beperkt. Modellen worden vaak getest op een vaste set categorieën \autocite{peng2021rp2klargescaleretailproduct}, terwijl assortimenten in supermarkten voortdurend veranderen.
...
\section{Positionering van dit onderzoek binnen de literatuur}
